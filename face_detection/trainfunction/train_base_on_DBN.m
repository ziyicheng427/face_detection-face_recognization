...
train_x=[];%%训练数据集
names = ["李柏潼","程子羿","纳比伊","尹睿昊"];

for i=1:4%----------------%数据库共有4人-------------------------------
    for j=1:5%-------------%每个人选择3张来进行训练-------------------------
        a=imread(strcat('C:\Users\22935\Desktop\face_recg(1) (2)\train\s',num2str(i),'_',num2str(j),'.bmp'));
        a=imresize(a,[120 120]);
        a=rgb2gray(a);
        imshow(a);
        disp(strcat('C:\Users\22935\Desktop\face_recg(1) (2)\train\s',num2str(i),'_',num2str(j),'.bmp'));
        c=a;
...
train_y=zeros(12,4);%%训练样本，输出标签,4*3=18
for i=1:4
    for j=1:5
        train_y((i-1)*5+j,i)=1;
    end
...
%-----------------------------------------------------------------------------
test_x=[];%%测试数据集
for i=1:4%----------------%ORL数据库共有40人-------------------------------
    for j=4:5%------------%每个人选择2张来进行测试-------------------------
        %-----------------数据库的路径------------------------------------
        a=imread(strcat('C:\Users\22935\Desktop\face_recg(1) (2)\train\s',num2str(i),'_',num2str(j),'.bmp'));
        a=imresize(a,[120 120]);
        a=rgb2gray(a);
...
num_labels=4;                        %所以，整个4层网络是input_layer_size-100-100-40
...
[dummy, expected] = max(test_y,[],2);
 ...
 save('C:\Users\22935\Desktop\face_recg(1) (2)\weights\weights.mat','Theta1','Theta2','Theta3');
 save('C:\Users\22935\Desktop\face_recg(1) (2)\names\names.mat','names')
%%
%--------------描绘关于隐层结点数的学习曲线----------------------------------
% lambda = 0.03;%正则化参数
